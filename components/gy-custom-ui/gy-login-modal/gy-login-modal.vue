<template>
	<div class="gy-login-modal">
		<tui-modal width="600rpx" radius="16rpx" padding="0" @cancel="show = false" :show="show" :custom="true">
			<view class="center_box">
				<p class="title">
					<span>一所勘测信息管理系统</span>
				...
				>
					{{ type === 'getUserInfo' ? '授权登录' : type === 'getPhoneNumber' && '获取手机号' }}
				</button>
			</view>
		</tui-modal>
	</div>
...
export default {
	...
	methods: {
		...
		all_fun(data, type) {
			if (data.detail.errMsg === 'getPhoneNumber:ok' || data.detail.errMsg === 'getUserInfo:ok') {
		...
		auth_login(data, type, open_id) {
			...
			this.$axios({
				...
				.then((res) => {
					...
					this.loading = false;
					if (res.code == 0) {
						this.common.toast(type == 1 ? '授权成功' : '获取成功', 500);
